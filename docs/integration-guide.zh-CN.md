...
`nativekit` 持有原生窗口和拖动会话，只能由 Electron 主进程导入。
...
import { drag, overlay } from '@zerob13/nativekit'
...
下面所有 preload 片段应合并进同一次 `exposeInMainWorld('desktop', ...)` 调用。
不要向 Renderer 暴露原始 `ipcRenderer`。
...
原生 drag loop 必须在主按钮仍按下时启动，所以 Renderer 用 `pointerdown` 立即
调用 preload；不能先等待另一个异步任务。
...
dragHandle.addEventListener('pointerdown', (event) => {
  ...
  void window.desktop.drag.start({ x: event.clientX, y: event.clientY })
})
```

```ts
// preload.ts (replace the earlier expose call with this combined object)
type DragResult = { dropped: boolean; x: number; y: number }

const dragApi = Object.freeze({
  start: (position: { x: number; y: number }) =>
    ipcRenderer.invoke('files:drag-out', position),
  onEnded: (callback: (result: DragResult) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, result: DragResult) =>
      callback(result)
    ipcRenderer.on('files:drag-ended', listener)
    return () => ipcRenderer.removeListener('files:drag-ended', listener)
  },
})
...
```ts
// main.ts
ipcMain.handle('files:drag-out', async (event, position) => {
  if (!win || event.sender !== win.webContents) {
    throw new Error('Unexpected IPC sender')
  }
  ...
  await drag.start({
    files: selectedAbsolutePaths,
    windowHandle: win.getNativeWindowHandle(),
    position,
  })
})

drag.on('ended', (result) => {
  win?.webContents.send('files:drag-ended', result)
})
```

`position` 是 BrowserWindow 内容区内的 DIP 坐标。一次只能有一个 drag session；
`ended` 返回 `dropped` 与最终屏幕 DIP 坐标。Renderer 卸载时必须调用 preload
返回的 unsubscribe。
...
- Finder/Explorer 的 drop 与 Escape cancel 都产生 `ended`；
...
| drag 立即取消/失败 | 调用发生在 `click` 或等待异步工作之后；改为直接响应主按钮 `pointerdown`。 |
